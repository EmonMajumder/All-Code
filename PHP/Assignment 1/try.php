...
if(isset($_POST['Page'])) {
    ...
    $sql = "select * from employees inner join salaries on employees.emp_no = salaries.emp_no where (salaries.to_date = '9999-01-01') and ((first_name like '%$name%') or (last_name like '%$name%'))";
...
{
    $Page =0 ;
    $result = mysqli_query($db, "select * from employees inner join salaries on employees.emp_no = salaries.emp_no where (salaries.to_date = '9999-01-01') and ((first_name like '%$name%') or (last_name like '%$name%')) limit $Page,$recordperpage");
}
if (!$result) {
...
<form id = "form1" action="<?php $_SERVER['PHP_SELF'] ?>"  method="post">
    ...
    if(isset($_POST['OrderbySalaryinput']))
        ...
        echo "";
    ?>"/>
    <input type="submit" onclick="orderby()" name="OrderbySalary" value="Sort by Salary"/>
</form>
<form name="LogoutForm" action="logOut.php" method="post">
    <input type="submit" name="logoutButton" value="Log Out"/>
</form>
